...
export class CartItemsService {
  ...
  async findByUser(user_id: number) {
    ...
    const cart = await this.cartItemsRepository.findCartByUser(user_id);

    if (!cart) {
      throw new NotFoundException('Cart not found');
    }

    return this.cartItemsRepository.findByUser(user_id, this.productSelect);
  }
  ...
  async findById(user_id: number, cart_item_id: number) {
    ...
    const item = await this.cartItemsRepository.findByIdForUser(
      user_id,
      cart_item_id,
      this.productSelect,
    );
  ...
  async remove(user_id: number, cart_item_id: number) {
    ...
    const result = await this.cartItemsRepository.removeForUser(
      user_id,
      cart_item_id,
    );
